...
export default function BlogsPage() {
  ...
  return (
    <div className="bg-background font-sans text-text antialiased">
        ...
        <div className="relative z-10 mx-auto grid max-w-7xl items-center gap-16 px-6 xl:grid-cols-[1.05fr_0.95fr]">
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: easeOut }} className="space-y-8">
            ...
            <h1 className="text-4xl font-bold leading-[1.05] tracking-tight text-white sm:text-5xl md:text-6xl xl:text-7xl">
              ...
              <span className="bg-linear-to-r from-primary via-primary to-accent bg-clip-text text-transparent">help you grow faster.</span>
            </h1>
            ...
            <div className="grid max-w-lg gap-4 sm:grid-cols-3">
              {[
                ...
                <div key={s.k} className="rounded-2xl border border-border bg-surface p-4">
            ...
            <Tilt3D>
              <SpotlightCard className="rounded-[2rem] border border-border bg-surface-elevated p-4 shadow-2xl shadow-black/50 backdrop-blur-xl">
                ...
                <div className="px-2 pb-2 pt-5">
                  ...
                  <p className="mt-3 leading-7 text-text-muted line-clamp-3">
                  ...
                  <div className="mt-5 flex items-center justify-between text-sm text-text-muted">
                    <span className="flex items-center gap-2"><FiUser className="h-4 w-4" /> {featured?.author?.name || 'GHLearning'}</span>
                  ...
                  <MagneticButton
                    ...
                    className="group mt-6 inline-flex items-center justify-center gap-2 rounded-full bg-linear-to-r from-primary to-accent px-7 py-3 text-sm font-semibold text-white shadow-lg shadow-primary/25"
                  >
                    Read full story
      ...
      {/* ============================ ARTICLES + SIDEBAR ============================ */}
      <section id="articles" className="bg-surface py-24">
        <div className="mx-auto max-w-7xl px-6">
          ...
          <Reveal>
            <div className="rounded-3xl border border-border bg-surface p-6 sm:p-8">
              <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
                <div>
                  <p className="text-sm uppercase tracking-[0.35em] text-primary">Browse topics</p>
                    ...
                    return (
                      <button
                        ...
                        className={`rounded-full px-4 py-2 text-sm font-medium transition-colors ${
                          active ? 'bg-linear-to-r from-primary to-accent text-white shadow-lg shadow-primary/20' : 'border border-border bg-surface-muted text-text-muted hover:border-primary/30 hover:text-primary'
                        }`}
                      >
                        {cat}
                      </button>
          ...
          <div className="mt-10 grid gap-10 xl:grid-cols-[1fr_320px]">
                ...
                <div className="grid gap-6 sm:grid-cols-2">
                  {Array.from({ length: 4 }).map((_, i) => (
                    <div key={i} className="overflow-hidden rounded-3xl border border-border bg-surface">
                      <div className="h-44 w-full animate-pulse bg-surface-muted" />
                          ...
                          <SpotlightCard className="flex h-full flex-col rounded-3xl border border-border bg-surface shadow-xl shadow-black/20">
                            ...
                            <div className="flex flex-1 flex-col p-6">
                              <Link href={`/pages/blogs/${article._id}`}>
                                <h3 className="text-xl font-semibold leading-snug text-white transition-colors hover:text-primary">{article.title}</h3>
                              </Link>
                              <p className="mt-3 flex-1 leading-7 text-text-muted line-clamp-3">{article.excerpt}</p>
                              <div className="mt-5 flex items-center justify-between border-t border-border pt-4 text-xs text-text-muted">
                                <span className="flex items-center gap-1.5"><FiUser className="h-3.5 w-3.5" /> {article.author?.name || 'GHLearning'}</span>
                                <span className="flex items-center gap-1.5"><FiClock className="h-3.5 w-3.5" /> {readLabel(article.readTime)}</span>
                              </div>
                              <Link href={`/pages/blogs/${article._id}`} className="mt-4 inline-flex items-center gap-1.5 text-sm font-semibold text-primary transition-colors hover:text-primary">
            ...
            <aside className="space-y-6">
              ...
              {popular.length > 0 && (
                <Reveal>
                  <SpotlightCard className="rounded-3xl border border-border bg-surface p-7">
                    ...
                    <div className="mt-6 space-y-3">
                      {popular.map((item, i) => (
                        <Link key={item._id} href={`/pages/blogs/${item._id}`} className="flex items-start gap-4 rounded-2xl border border-border bg-black/40 p-4 transition-colors hover:border-primary/30">
                          <span className="bg-linear-to-r from-primary to-accent bg-clip-text text-2xl font-bold text-transparent">{String(i + 1).padStart(2, '0')}</span>
                          <div>
                            <p className="text-sm font-medium leading-snug text-text line-clamp-2">{item.title}</p>
                            <p className="mt-1 text-xs text-text-subtle">{(item.views || 0).toLocaleString()} reads</p>
                          </div>
                        </Link>
                      ))}
                    </div>
              ...
              <Reveal delay={0.05}>
                <SpotlightCard className="rounded-3xl border border-border bg-surface p-7">
                  <p className="text-sm uppercase tracking-[0.3em] text-primary">Quick stats</p>
                  <div className="mt-6 grid grid-cols-2 gap-4">
                    {stats.map((s) => (
                      <div key={s.label} className="rounded-2xl bg-black/40 p-5 text-center">
                        <p className="bg-linear-to-r from-primary to-accent bg-clip-text text-3xl font-bold text-transparent">
                          <CountUp to={s.to} />
                          {s.suffix}
                        </p>
          ...
          <Reveal>
            ...
            >
              ...
              <div className="relative">
                <h2 className="mx-auto max-w-3xl text-4xl font-bold tracking-tight text-text sm:text-5xl">Want more guides, tutorials, and course stories?</h2>
