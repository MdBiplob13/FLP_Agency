...
const roleStyles = {
  user: 'text-slate-300 bg-slate-500/10 ring-slate-500/20',
  teacher: 'text-emerald-300 bg-emerald-500/10 ring-emerald-500/20',
  admin: 'text-blue-300 bg-blue-500/10 ring-blue-500/20',
    ...
    <div>
      ...
      <div>
        ...
        <p className="mt-2 text-slate-400">
          ...
          {!isSuperadmin && (
            <span className="ml-1 text-slate-500">Admin promotions are superadmin-only.</span>
          )}
        </p>
      </div>
      ...
      <div className="mt-6 overflow-hidden rounded-3xl border border-white/10 bg-slate-950/60">
        {loading ? (
          ...
          <div className="py-20 text-center">
            <FiUsers className="mx-auto h-10 w-10 text-slate-600" />
            <p className="mt-4 text-lg font-semibold text-white">No users found</p>
            <p className="mt-1 text-slate-400">Try another search term or role filter.</p>
          </div>
        ) : (
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <img
                            src={u.photo}
                            alt={u.name}
                            className="h-10 w-10 flex-none rounded-full object-cover ring-2 ring-white/10"
                          />
                        ...
                        <div className="flex items-center justify-end gap-2">
                          {u.role === 'user' && manageable && (
                            <button
                              ...
                              disabled={saving}
                              className="hidden cursor-pointer items-center gap-1.5 rounded-full border border-emerald-400/30 bg-emerald-500/10 px-3 py-1.5 text-xs font-semibold text-emerald-300 transition-colors hover:bg-emerald-500/20 disabled:cursor-not-allowed disabled:opacity-60 sm:inline-flex"
                            >
                          ...
                          {manageable ? (
                            <button
                              ...
                              className="inline-flex cursor-pointer items-center gap-1.5 rounded-full border border-white/10 bg-white/5 px-4 py-1.5 text-xs font-semibold text-slate-200 transition-colors hover:border-blue-500/40 hover:text-blue-200"
                            >
                              Manage
                            </button>
                          ) : (
                            <span
                              ...
                              className="inline-flex items-center gap-1.5 rounded-full border border-white/5 bg-white/[0.02] px-4 py-1.5 text-xs font-medium text-slate-600"
                            >
      ...
      {!loading && users.length > 0 && pagination.totalPages > 1 && (
        <div className="mt-6 flex flex-col items-center justify-between gap-4 sm:flex-row">
          <p className="text-sm text-slate-400">
            ...
            of <span className="font-semibold text-slate-200">{pagination.total}</span>
          </p>
          ...
          <div className="flex items-center gap-1.5">
            <button
              ...
              className="inline-flex h-9 w-9 items-center justify-center rounded-full border border-white/10 bg-white/5 text-slate-300 transition-colors enabled:cursor-pointer enabled:hover:border-blue-500/40 enabled:hover:text-blue-200 disabled:cursor-not-allowed disabled:opacity-40"
            >
            ...
            {getPageItems(pagination.page, pagination.totalPages).map((it, i) =>
              it === '…' ? (
                <span key={`gap-${i}`} className="px-1.5 text-sm text-slate-600">
                  …
                </span>
              ) : (
                <button
                  ...
                  className={`inline-flex h-9 min-w-9 items-center justify-center rounded-full px-3 text-sm font-semibold transition-colors ${
                    it === pagination.page
                      ? 'bg-linear-to-r from-blue-600 to-purple-600 text-white'
                      : 'cursor-pointer border border-white/10 bg-white/5 text-slate-300 hover:border-blue-500/40 hover:text-blue-200'
                  }`}
                >
                  {it}
                </button>
          ...
          <div className="relative w-full max-w-lg rounded-3xl border border-white/10 bg-[#0a0a12] shadow-2xl shadow-black/50">
            ...
            <div className="space-y-6 px-6 py-6">
              ...
              <div className="flex items-center gap-4">
                ...
                />
                <div className="min-w-0">
                  <p className="truncate text-lg font-semibold text-white">{manageTarget.name}</p>
                  <span
              ...
              {/* Contact details */}
              <div className="space-y-2 rounded-2xl border border-white/10 bg-slate-900/40 px-4 py-4 text-sm text-slate-300">
                <p className="flex items-center gap-2.5">
                  <FiMail className="h-4 w-4 flex-none text-slate-500" />
                  <span className="truncate">{manageTarget.email}</span>
                </p>
                <p className="flex items-center gap-2.5">
                  <FiPhone className="h-4 w-4 flex-none text-slate-500" />
            ...
            <div className="flex items-center justify-end gap-3 border-t border-white/10 px-6 py-5">
              ...
              <button
                ...
                className="inline-flex cursor-pointer items-center gap-2 rounded-full bg-linear-to-r from-blue-600 to-purple-600 px-6 py-2.5 text-sm font-semibold text-white transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-50"
              >
